...
import { MonthPicker, MemberPicker, StatusChip, prettyDate, TypeTag, HonorBadges, BirthdayCake } from "./Shared.jsx";
...
import {
  ...
  monthKey, monthLabel, memberName, memberHonors, birthdayLabel, isBirthdayToday, isHonorary,
} from "../lib/stats.js";
...
export default function MemberPage({ model, memberId, mk, months, setMonth, today, go }) {
              ...
              <li key={m.member_id}>
                <button onClick={() => go(`#/members/${m.member_id}`)}>
                  <strong>
                    ...
                    <BirthdayCake member={m} today={today} />
                  </strong>{" "}
                  <span>{memberName(m).full}</span>
                  <HonorBadges member={m} />
                </button>
              </li>
            ))}
          </ul>
  ...
  const honorary = isHonorary(member);
  const honors = memberHonors(member);
  const bday = birthdayLabel(member);
  const bdayToday = isBirthdayToday(member, today);
      ...
      <section className="card member-hero">
        <div className="member-hero__head">
          <div>
            ...
            <h1>
              {n.nickname}
              <BirthdayCake member={member} today={today} />
            </h1>
            <p className="muted">
              {n.full}
              {honors.classification ? ` · ${honors.classification}` : ""}
              {inactive ? " · Inactive" : ""}
            </p>
            <HonorBadges member={member} />
            {bday && (
              <p className={"member-bday" + (bdayToday ? " member-bday--today" : "")}>
                🎂 Birthday: {bday}
                {bdayToday ? " — happy birthday!" : ""}
              </p>
            )}
          </div>
        ...
        <div className="member-hero__ring">
          ...
          {honorary ? (
            <span className="chip chip--hon">Honorary member — no attendance requirement</span>
          ) : (
          ...
          {!honorary && stat.status === "needs" && (
            <p className="member-hero__hint">
              {stat.remaining} more meeting{stat.remaining > 1 ? "s" : ""} or makeup
              activit{stat.remaining > 1 ? "ies" : "y"} will complete this month
              (the requirement is half of this month's scheduled meetings).
            </p>
          )}

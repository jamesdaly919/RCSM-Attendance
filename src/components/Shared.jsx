...
import { monthLabel, memberName, memberHonors, isBirthdayToday } from "../lib/stats.js";

// 🎂 shown beside a member's name on their birthday.
export function BirthdayCake({ member, today }) {
  if (!isBirthdayToday(member, today)) return null;
  return (
    <span className="bday-cake" title="It's their birthday today!" aria-label="Birthday today">
      🎂
    </span>
  );
}

// Small public badges: club position, Past President, and recognitions
// (Paul Harris Fellow, Major Donor, …). Recognition matters to members,
// so every recognition is shown by its full name with its 🎖️ icon,
// visible to everyone, everywhere a member appears.
export function HonorBadges({ member }) {
  const h = memberHonors(member);
  const badges = [];
  if (h.position) badges.push(["pos", "🔹", h.position]);
  if (h.pastPresident) badges.push(["pp", "🏵️", `Past President ${h.pastPresident}`]);
  if (h.honorary) badges.push(["hon", "🕊️", "Honorary Member"]);
  for (const r of h.recognitions) badges.push(["phf", "🎖️", r]);
  if (badges.length === 0) return null;
  return (
    <span className="honor-badges">
      {badges.map(([kind, icon, label], i) => (
        <span key={i} className={`badge badge--${kind}`}>
